...
public class SocialSearchResults extends AbstractSearchResults<SocialSearchResult>
{
	...
	{	logger.log("Starting filtering the articles");
		...
		logger.increaseOffset();
			...
			if(searchDate)
				txt = txt + "(dates are ignored here, because they were already used during the social search)";
			logger.log(txt);
	...
	{	logger.log("Recording all the social search results in a single file");
		logger.increaseOffset();
